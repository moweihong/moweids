...
class TypeModel extends Model
{
   ...
   public function getTypeBrandList($condition, $field = '*') {
        return M()->table(C('DB_PREFIX').'type_brand')->field($field)->where($condition)->select();
    }

    public function getGoodsAttrIndexList($conditoin,$page = 0, $fields = '', $order = '', $limit = '') {
        return M()->table(C('DB_PREFIX').'goods_attr_index')->where($conditoin)->order($order)->limit($limit)->page($page)->select();
    }
    ...
    public function getAttr($type_id, $store_id, $gc_id) {
        $spec_list = $this->typeRelatedJoinList(array('type_id' => $type_id), 'spec', 'allwood_spec.sp_id as sp_id, allwood_spec.sp_name as sp_name');        
        $attr_list = $this->typeRelatedJoinList(array('allwood_attribute.type_id' => $type_id), 'attr', 'allwood_attribute.attr_id as attr_id, allwood_attribute.attr_name as attr_name, allwood_attribute_value.attr_value_id as attr_value_id, allwood_attribute_value.attr_value_name as attr_value_name');
        $brand_list = $this->typeRelatedJoinList(array('type_id' => $type_id, 'brand_apply' => 1), 'brand', 'allwood_brand.brand_id as brand_id,allwood_brand.brand_name as brand_name');
        // 整理数组
        $spec_json = array();
        if (is_array($spec_list) && !empty($spec_list)) {
            $array = array();
            foreach ($spec_list as $val) {
                $spec_value_list = D('Spec')->getSpecValueList(array('sp_id'=>$val['sp_id'], 'gc_id'=>$gc_id, 'store_id' => $store_id));
                $a = array();
        ...
        if (empty($brand_list)) {
            ...
            $brand_list = D('Brand')->getBrandList($condition);
        }
    ...
    public function delGoodsAttr($conditoin) {
        return M()->table(C('DB_PREFIX').'goods_attr_index')->where($conditoin)->delete();
    }
	/**
	 * 类型列表
	 * @param array  $param 
    ...
    public function typeGoodsRelatedAdd($param, $table, $type = "") {
        if (is_array ( $param ['value'] ) && ! empty ( $param ['value'] )) {
            ...
            M()->table(C('DB_PREFIX').$table)->addAll($insert_array);
        }
    }
	/**
	 * 对应关系信息列表
...
//		$list_type		= Db::select($array);
		$list_type		= $this->table(C('DB_PREFIX').$table)->where($condition_str)->field($field)->order($param['order'])->select();
		return $list_type;
	}
	/**
	 * 更新属性信息
    ...
    public function typeRelatedJoinList($param, $type = '', $field = '*', $order = '') {
        ...
        switch ($type) {
            case 'spec':
                $table = 'allwood_type_spec';
                $join = 'allwood_spec on allwood_type_spec.sp_id=allwood_spec.sp_id';
                $order = !empty($order) ? $order : 'allwood_spec.sp_id asc, allwood_spec.sp_sort asc';
                break;
            case 'attr':
                $table = 'allwood_attribute';
                $join = 'allwood_attribute_value on allwood_attribute.attr_id=allwood_attribute_value.attr_id';
                $on = 'allwood_attribute.attr_id=allwood_attribute_value.attr_id';
                $order = !empty($order) ? $order : 'allwood_attribute.attr_sort asc, allwood_attribute_value.attr_value_sort asc, allwood_attribute_value.attr_value_id asc';
                break;
            case 'brand':
                $table = 'allwood_type_brand';
                $join = 'allwood_brand on allwood_type_brand.brand_id=allwood_brand.brand_id';
                $order = !empty($order) ? $order : 'allwood_brand.brand_sort asc';
                break;
        }
       // $result = $this->field($field)->join($join)->where($param)->order($order)->select();
        $result = M()->table($table)->join($join)->field($field)->where($param)->order($order)->select();
        return $result;
    }
	/**
	 * 类型与属性关联信息,多表查询
	...
	private function getCondition($condition_array) {
		...
		return ltrim($condition_str, ' and ');
	}
}
