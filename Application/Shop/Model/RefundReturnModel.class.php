...
class RefundReturnModel extends Model{
	...
	public function getRefundReturnList($condition = array(), $page = '', $fields = '*', $limit = '') {
		...
		$result = $this->table('allwood_refund_return')->field($fields)->where($condition)->page($page)->limit($limit)->order('refund_id desc')->select();
		return $result;
	}
     ...
     * @return array
     */
    public function getReturnList2($condition = array(), $page = '') {
        $condition['refund_type'] = '1';//类型:1为退款,2为退货
        $result = $this->getRefundReturnList($condition, $page);
        return $result;
    }
	...
	public function getGoodsRefundList($order_list = array()) {
	    ...
	    $refund_list = $this->table('allwood_refund_return')->where($condition)->order('refund_id desc')->select();
        	    ...
        	    if ($order_state == $order_paid && $payment_code != 'offline') {//已付款未发货的非货到付款订单可以申请取消
            	    ...
            	    foreach ($goods_list as $k => $v) {
            	        ...
            	        if (!empty($refund_goods[$order_id][$goods_id])) {
                            ...
                            $return_type = $refund_goods[$order_id][$goods_id]['return_type'];
                            $refund_type = $refund_goods[$order_id][$goods_id]['refund_type'];
                            $refund_state = $refund_goods[$order_id][$goods_id]['refund_state'];
                            //卖家处理状态:1为待审核,2为同意,3为不同意
                            $v['seller_state'] = $seller_state;
                            $v['return_type'] = $return_type;
                            $v['refund_type'] = $refund_type;
                            $v['refund_state'] = $refund_state;
            	            if ($seller_state == 3) {
    ...
    public function getGoodsRefundList2($order_list = array()) {
        $order_ids = array();//订单编号数组
        $order_ids = array_keys($order_list);
        $model_trade = Model('trade');
        $condition = array();
        ...
        if (!empty($refund_list) && is_array($refund_list)) {
            foreach ($refund_list as $key => $value) {
                $order_id = $value['order_id'];//订单编号
                $goods_id = $value['order_goods_id'];//订单商品表编号
                //发货前退款的商品是没有没有goods_id
                //的，需要处理
                if($goods_id == 0){
                    $order_goods_model = Model('goods');
                    unset($condition);
                    $condition['order_id'] = $order_id;
                    $goods_list1 = $order_goods_model->table('order_goods')->field('rec_id')->where($condition)->select();
                    //遍历订单商品id号码，将退款订单填充进去
                    foreach ($goods_list1 as $key1 => $value1) {
                        if (empty($refund_goods[$order_id][$value1['rec_id']])) {
                            $refund_goods[$order_id][$value1['rec_id']] = $value;
                        }    
                    }
                }else{
                    if (empty($refund_goods[$order_id][$goods_id])) {
        ...
        if (!empty($order_list) && is_array($order_list)) {
            foreach ($order_list as $key => $value) {
                ...
                $order_paid = $model_trade->getOrderState('order_paid');//订单状态20:已付款
                $payment_code = $value['payment_code'];//支付方式
                if (10000 == $order_paid && $payment_code != 'offline') {//已付款未发货的非货到付款订单可以申请取消
                    $order_list[$order_id]['refund'] = '1';
                } elseif ( !empty($goods_list) && is_array($goods_list)) {//已发货后对商品操作
                    $refund = $this->getRefundState($value);//根据订单状态判断是否可以退款退货
                    if($order_id == 441){
                    }else{
                    }
     ...
     * @return array
     */
    public function getGoodsRefundListBuyer($order_list = array()) {
        $order_ids = array();//订单编号数组
        $order_ids = array_keys($order_list);
        $model_trade = Model('trade');
        $condition = array();
        $condition['order_id'] = array('in', $order_ids);
        //区分发货前退款和发货后退款 0 为发货前 1 为发货后
        //同时，要防止发货前的退款单 在卖家发货后进入 另外一种状态
        ...
        //考虑到退款申请可以重复多次申请，故对结果时间升序排列，覆盖旧版本
        $refund_list = $this->table('refund_return')->where($condition)->order('add_time asc')->select();
        if(!empty($refund_list)){
            foreach ($refund_list as $key1 => $value1) {
                $refund_list1[$value1['order_id']] = $value1;
            }
        }
        if (!empty($order_list) && is_array($order_list)) {
            foreach ($order_list as $key => $value) {
                $order_list[$key]['moneyback'] = $refund_list1[$key];
            }
        }
        return $order_list;
    }
    ...
    public function getRefundReturnCount($condition) {
        ...
        $conditio['tesu_deleted'] = '0';
        
        return $this->table('allwood_refund_return')->where($condition)->count();
    }
