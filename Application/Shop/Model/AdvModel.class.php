...
class AdvModel extends Model{
   ...
   /**
	...
	/*获取所有广告id*/
	public function getAllAdvId(){
		$model=model();
		$advids=$model->table('adv_position')->field('ap_id')->select();
		return $advids;
	}
	...
	public function makeApAllCache(){
        ...
        public function getAdvList($condition){	
		$adv_list =$this->where($condition)->order('slide_sort, adv_id desc')->select();
                foreach($adv_list as $k=>$v){
                    if($v['adv_content']){
                        $adv_list[$k]['adv_content']=  unserialize($v['adv_content']);
                        if($adv_list[$k]['adv_content']['adv_pic']){
                            $adv_list[$k]['adv_content']['thumb_img']=substr($adv_list[$k]['adv_content']['adv_pic'],0,-4)."_small.jpg" ;
                        }
        ...
        public function getAdvInfo($condition){
		$model = model();		
		$advInfo =$this->table($this->tablePrefix.'adv')->where($condition)->find();                
                if($advInfo['adv_content']){
	...
	public function getApInfo($condition,$img=false){
		 $sql=M('adv_position')->where($condition);
		if($img){
			$info=$sql->getField('default_content');
		}else{
			$info=$sql->find(); 
		}		                        
		return $info;
