...
class SpecModel extends Model
{
    ...
    public function getSpecValueList($where, $field = '*', $order = 'sp_value_sort asc,sp_value_id asc') {
        $result = M()->table('allwood_spec_value')->field($field)->where($where)->order($order)->select();
    ...
    public function addSpecValue($insert) {
        $result = M('spec_value')->add($insert);
        return $result;
    }
    ...
    public function addSpecValueALL($insert) {
        $result = $this->table($this->tablePrefix.'spec_value')->insertAll($insert);
        return $result;
    }
    ...
    public function delSpecValue($where) {
        $result = $this->table($this->tablePrefix.'spec_value')->where($where)->delete();
        return $result;
    }
	/**
	 * 更新规格信息
    ...
    /**
     * 根据一组class_id获取相关的所有规格
     *
     * @param	array $condition
     * @return	string
     */

    public function getSpecLists($condition){
