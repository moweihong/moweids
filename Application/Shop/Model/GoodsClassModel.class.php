...
class GoodsClassModel extends Model
{
    ...
    protected $trueTableName = 'allwood_goods_class';
   ...
   /**
    ...
    {
        $result = $this->field($field)->where($condition)->order('gc_parent_id asc,gc_sort asc,gc_id asc')->limit(10000)->select();
        return $result;
    }
     ...
     * @param   number $on_cache 1:读取缓存 0：读数据库
     * @return  array   二维数组
     */
    public function getGoodsClass($store_id, $pid = 0, $deep = 1, $on_cache = 0)
    {
        // 读取商品分类
        ...
        if ($on_cache == 1){
            ...
            if (!empty($goods_class)){
                foreach ($goods_class as $k => $v){
                    if ($pid == $v['gc_parent_id'] && $deep == $v['depth']){
                        $gc_list[] = $v;
                    }
                }
            }
            ...
            $model_storebindclass = D('StoreBindClass');
    ...
    public function get_all_category($update_all = 0){
        ...
        if (!file_exists($file_name) || $update_all == 1) {//文件不存在时更新或者强制更新时执行
            ...
            if (is_array($class_list) && !empty($class_list)) {
                ...
                $hot_word_model = Model('hot_word');
                $hot_word_list = $hot_word_model->getHotWordList();
                ...
                foreach ($class_list as $key => $value) {
                    ...
                    if ($p_id == 0) {//第1级分类
                        $gc_list[$gc_id] = $value;
                        $gc_list[$gc_id]['hot_word'] = $hot_word_list[$gc_id];
    ...
    public function getClassList($condition, $field = '*')
    {
        ...
        $param['order'] = $condition['order'] ? $condition['order'] : 'gc_deep asc, gc_parent_id asc,gc_sort asc,gc_id asc';
        ...
        if (is_array($class_list) && !empty($class_list)) {
            ...
            for ($i; $i < $size; $i++) {//$i为上次循环到的分类编号，避免重新从第一条开始
                ...
                if ($gc_parent_id == $parent_id) {
                    $val['deep'] = $deep;
                    $show_class[] = $val;
                    if ($deep < $show_deep && $deep < 5) {//本次深度小于显示深度时执行，避免取出的数据无用
                        $this->_getTreeClassList($show_deep, $class_list, $deep + 1, $gc_id, $i + 1);
                    }
                }
                //此处的假设是parentid 可以反应层级关系 当选中项的gc_parent_id 大于父id的，
                //可以认为后面不会有gc_parent和父id相等所以会终止
     ...
     **/
    public function findArrClass($condition){
     ...
     * 找到二级分类和三级分类
     */
    public function findLowLevel($gc_id)
    {
        //获取二级分类
        $condition = array('order' => 'gc_parent_id asc,gc_sort asc,gc_id asc', 'gc_parent_id'=>$gc_id);
        $second_class = $this->getClassList($condition);
        $return_class=array();
        foreach ($second_class as $key=>$val){
            $return_class[$key] = $val;
            $gc_id = $val['gc_id'];
            $pic_name = BASE_UPLOAD_PATH . '/' . ATTACH_COMMON . '/category-pic-' . $gc_id . '.jpg';
            if (file_exists($pic_name)) {
                $return_class[$key]['pic'] = UPLOAD_SITE_URL . '/' . ATTACH_COMMON . '/category-pic-' . $gc_id . '.jpg';
            }
            //查找三级分类
            $condition = array('order' => 'gc_parent_id asc,gc_sort asc,gc_id asc', 'gc_parent_id'=>$val['gc_id']);
            $three_class = $this->getClassList($condition);
            $return_class[$key]['class3'] = $three_class;
        }
        return $return_class;
    }
    /*
     * 返回顶级分类id
     */
    public function findTopClass($gc_id,&$class = array()){
        $class = $this->getOneGoodsClass($gc_id);
        if($class['gc_parent_id'] != 0){
            $this->findTopClass($class['gc_parent_id'],$class);
        }
        return $class['gc_id'];
    }
    /*
     * 返回顶级分类名称
     */
    public function findTopClassName($gc_id,&$class = array()){
        $class = $this->getOneGoodsClass($gc_id);
        if($class['gc_parent_id'] != 0){
            $this->findTopClass($class['gc_parent_id'],$class);
        }
	...
	public function H(){
	    ...
	    $result = $this->where(array('tesu_deleted' => 0))->field($fields)->order('gc_parent_id asc, gc_sort asc')->limit(10000)->select();
	 ...
	 * 指定id下的商品分类子分类
	 */
	public  function getGoodsCate($cid){
		$cate=$this->where(array('gc_parent_id'=>$cid))->select();
		if($cate){
			foreach($cate as $k1=>$v1){
				$cate2=$this->where(array('gc_parent_id'=>$v1['gc_id']))->select();
				if($cate2){
					$cate[$k1]['class3']=$cate2;
				}
			}
		}	
		return $cate;
