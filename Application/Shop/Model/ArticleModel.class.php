...
class articleModel{
	...
	//获取某一分类下的文章
	public function getArticleListNew($ac_id)
	{
		$ac_name=M('ArticleClass')->field('ac_name')->find($ac_id);
		$result=M('Article')->where('tesu_deleted=0 and ac_id='.$ac_id)->order('article_id desc')->limit(0,10)->select();
		$result['ac_name']=$ac_name['ac_name'];
		return $result;
	}
