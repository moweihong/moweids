...
class ArticleclassModel{
	...
	public function getClassList($condition){
		...
		$param = array();
		$param['table'] = 'allwood_article_class';
	...
	public function getOneClass($id){
		if (intval($id) > 0){
			$result = M('article_class')->find($id);
			return $result;
		}else {
			return false;
		}
	...
	public function addrecord($param){
		...
		if (is_array($param)){
			...
			$result = M('Article_class')->data($param)->add();
			return $result;
		}else {
			return false;
		}
	...
	public function update($param){
		...
		if (is_array($param)){
			...
			$result = M('article_class')->where($where)->save($param);
			return $result;
		}else {
			return false;
		}
	...
	public function del($id){
		if (intval($id) > 0){
			...
			$result = M('Article_class')->where($where)->delete();
			return $result;
		}else {
			return false;
		}
