...
class alipay{
	...
	public function notify_verify() {
        log::i(__METHOD__, 'ali');
		$param	= $_POST;
        log::i('post = '.encode_json($_POST), 'ali');
        ...
        log::i(' notify url = '.$veryfy_url, 'ali');
		$veryfy_result  = $this->getHttpResponse($veryfy_url);
        log::i(' notify result = '.$veryfy_result, 'ali');
		$mysign = $this->sign($param);
        log::i(' sign = '.$mysign, 'ali');
        if(!preg_match("/true$/i",$veryfy_result)){
            log::i(' notify result not match ', 'ali');
        }else{
            log::i(' notify result is match!', 'ali');
        }

        if(!($mysign == $param["sign"])){
            log::i(' sign is not equal  parm sign = '.$param['sign'].' mysign ='.$mysign, 'ali');
        }else{
            log::i(' sign is qual ', 'ali');
        }
		if (preg_match("/true$/i",$veryfy_result) && $mysign == $param["sign"])  {
		    $this->order_type = $param['extra_common_param'];
            log::i(' notify process complete'.'order type = '.$this->order_type, 'ali');
			return true;
		} else {
			return false;
		}
	...
	public function return_verify() {
        log::i(__METHOD__, 'ali');
		$param	= $_GET;
        log::i('get = '.encode_json($_GET), 'ali');
		$param['act']	= "";//将系统的控制参数置空，防止因为加密验证出错
		$param['op']	= "";
		$param['key']	= $this->payment['payment_config']['alipay_key'];
		$veryfy_url = $this->alipay_verify_url. "partner=" .$this->payment['payment_config']['alipay_partner']. "&notify_id=".$param["notify_id"];
        log::i(' verify url = '.$veryfy_url, 'ali');
		$veryfy_result  = $this->getHttpResponse($veryfy_url);
        log::i(' notify result = '.$veryfy_result, 'ali');
		$mysign = $this->sign($param);
        if(!preg_match("/true$/i",$veryfy_result)){
            log::i(' verity result not match ', 'ali');
        }else{
            log::i(' verify result is match!', 'ali');
        }
		...
		if (preg_match("/true$/i",$veryfy_result) && $mysign == $param["sign"])  {
            $this->order_type = $param['extra_common_param'];
            log::i(' verify process complete'.'order type = '.$this->order_type, 'ali');
            return true;
		} else {
			return false;
		}
