...
class PayNotifyCallBack extends WxPayNotify
{
	...
	public function NotifyProcess($data, &$msg)
	{
		...
		if (in_array($data['attach'],array('predeposit','product_buy','store_fund', 'store_operate_fund'))) {
		    $order_type = $data['attach'];
		} else {
		    $order_pay_info = Model('order')->getOrderPayInfo(array('pay_sn'=> $data['out_trade_no']));
		    if(empty($order_pay_info)){
		        $order_type = 'predeposit';
		    } else {
		...
		if ($order_type == 'product_buy') {
		...
		} else if($order_type == 'pd_rechange') {
		    ...
		    $api_pay_amount = $order_info['order_amount'] - $order_info['pd_amount'] - $order_info['rcb_amount'];
		} else if($order_type == 'store_fund') {

		} else if($order_type == 'store_operate_fund') {
