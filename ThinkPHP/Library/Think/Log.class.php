...
class Log {
    ...
    static function write($message,$level=self::ERR,$type='',$destination='') {
     ...
     /**
     * 写日志(linux 下输入日志：鲁仕鑫专用)
     */
    static function lol($message,$level=self::ERR,$type='',$destination='') {
        if(!self::$storage){
            $type   =   $type ? : C('LOG_TYPE');
            $class  =   'Think\\Log\\Driver\\'. ucwords($type);
            $config['log_path'] = C('LOG_PATH');
        ...
        if(empty($destination)){
            if(PHP_OS == "Linux"){
                $destination = '/data001/www/logs/tp_wood/'.date('y_m_d').'.log';            
            }else{
                $destination = C('LOG_PATH').date('y_m_d').'.log';
            }
            ...
            if(PHP_OS == "Linux"){
                $destination = '/data001/www/logs/tp_wood/'.date('y_m_d').$destination.'.log';            
            }else{
                $destination = C('LOG_PATH').date('y_m_d').$destination.'.log';
            }
        }
        self::$storage->write("{$level}: {$message}", $destination);
    }
     ...
     * 写日志(linux 下输入日志：鲁仕鑫专用)
     */
    static function ext_log($message,$destination='',$level=self::ERR,$type='') {
        if(!self::$storage){
            $type   =   $type ? : C('LOG_TYPE');
            $class  =   'Think\\Log\\Driver\\'. ucwords($type);
            $config['log_path'] = C('LOG_PATH');
