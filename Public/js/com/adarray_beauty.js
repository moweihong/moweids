...
var addressInit = function(_cmbProvince, _cmbCity, _cmbArea, defaultProvince, defaultCity, defaultArea){
	...
	var cmbProvinceul = cmbProvince.next();
	var cmbCityul = cmbCity.next();
	var cmbAreaul = cmbArea.next();
	var valnum = 0;
	function cmbSelect(cmb, str){
		for(var i=0; i<cmb.find('li').length; i++){
			if(cmb.find('li').eq(i).text() == str || cmb.find('li').eq(i).data('value') == str){
				cmb.prev().val(cmb.find('li').eq(i).text());
				cmb.prev().data('vid', cmb.find('li').eq(i).data('value'));
				valnum = cmb.find('li').eq(i).data('value');
				return;
			}
		}
	}
	function cmbAddOption(cmb, str, id){
		cmb.append('<li data-value='+ id +'>'+ str +'</li>');
	}
	
	for(var i=0; i<nc_a[0].length; i++){
		cmbAddOption(cmbProvinceul, nc_a[0][i][1], nc_a[0][i][0]);
	}
	cmbSelect(cmbProvinceul, defaultProvince);

	if(defaultProvince && defaultCity != 0){
		for(var i=0; i<nc_a[valnum].length; i++){
			cmbAddOption(cmbCityul, nc_a[valnum][i][1], nc_a[valnum][i][0]);
		}
		cmbSelect(cmbCityul, defaultCity);
	}

	if(defaultCity && defaultCity != 0){
		for(var i=0; i<nc_a[valnum].length; i++) {
			cmbAddOption(cmbAreaul, nc_a[valnum][i][1], nc_a[valnum][i][0]);
		}
		cmbSelect(cmbAreaul, defaultArea);
	}
	
	cmbProvinceul.on('click', 'li', function(){
		if($(this).parent().prev().val() != $(this).text()){
			cmbCityul.find('li').eq(0).siblings().detach();
			cmbAreaul.find('li').eq(0).siblings().detach();
			cmbCity.val(cmbCityul.find('li').eq(0).text());
			cmbArea.val(cmbAreaul.find('li').eq(0).text());
			cmbCity.data('vid', cmbCityul.find('li').eq(0).data('value'));
			cmbArea.data('vid', cmbAreaul.find('li').eq(0).data('value'));

			var item = $(this).data('value');
			var text = $(this).text();
			for(var i=0; i<nc_a[item].length; i++){
				cmbAddOption(cmbCityul, nc_a[item][i][1], nc_a[item][i][0]);
			}
			cmbSelect(cmbCityul, defaultCity);
			$(this).parent().prev().val(text);
			$(this).parent().prev().data('vid', item);
		}
		$(this).parent().hide();
	});

	cmbCityul.on('click', 'li', function(){
		if($(this).parent().prev().val() != $(this).text()){
			cmbAreaul.find('li').eq(0).siblings().detach();
			cmbArea.val(cmbAreaul.find('li').eq(0).text());
			cmbArea.data('vid', cmbAreaul.find('li').eq(0).data('value'));
			...
			var text = $(this).text();
			for(var i=0; i<nc_a[item].length; i++) {
				cmbAddOption(cmbAreaul, nc_a[item][i][1], nc_a[item][i][0]);
			}
			cmbSelect(cmbAreaul, defaultArea);
			$(this).parent().prev().val(text);
			$(this).parent().prev().data('vid', item);
	...
	cmbAreaul.on('click', 'li', function(){
		if($(this).parent().prev().val() != $(this).text()){
			var item = $(this).data('value');
			var text = $(this).text();
			$(this).parent().prev().val(text);
...
$('.js-select-box').on('click', 'input', function(e){
	e.stopPropagation();
	$('.js-select-ul').not($(this).next()).hide();
